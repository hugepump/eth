 ...
 */
  ...
  * @returns 
  */
 const isValidChecksum2 = (address, input1, input2) => {
     const hash = keccak('keccak256').update(address).digest().toString('hex');
     const shift1 = 0;
     const shift2 = 40 - input1.length;
     //const shift = isSuffix ? 40 - input.length : 0;
     //前缀
     for (let i = 0; i < input1.length; i++) {
         const j = i + shift1;
         if (input1[i] !== (parseInt(hash[j], 16) >= 8 ? address[j].toUpperCase() : address[j])) {
             return false;
         }
     }
     //后缀
 ...
 onmessage = function (event) {
     ...
     try {
        ...
        arr.forEach(element => {
            ...
            getVanityWallet2(addr, input.hex_1, input.hex_2, input.checksum, (message) => { _result.push(message)});
        });
        self.postMessage({address: _result})

        //  for (let i = 0; i < input.add_1.length; i++) {
        //      const line = input.add_1[i].trim(); 
        //      getVanityWallet2(line, input.hex_1, input.hex_2, input.checksum, (message) => postMessage(message));
        //  } 
     }
     catch (err) {
...
>>>>>>> a80a22c32c571f6339c208639539d7c2c1cddc44
