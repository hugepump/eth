...
<script>
    ...
    export default {
        ...
        methods: {
            save() {
                if (this.password) {
                    ...
                    setTimeout(() => {
                        ...
                        const fileName = 'UTC--' + new Date().toISOString().replace(/:/g, '-') + '--' ;
                        download(JSON.stringify(this.result), fileName, 'application/json');
                        this.loading = false;
                    }, 20);
                }
            },
            ...
            generateWallet(privateKey, password, address) {
